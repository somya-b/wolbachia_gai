...
run.model <- function(i){
  treatment.identifier <- donor[i]
  controls.identifier <- donor[!donor %in% c(treatment.identifier)]
  t <- map.t$date[map.t$ey.ew == 2020.01]
...
intermediate_directory <- 'RESULTS/CONTROL'

out <- foreach(i=1:length(donor), .packages = c('MSCMT','dplyr','data.table')) %dopar% {
